...
将 Java 源码解析为 PlantUML 类图文本（依赖 [javalang](https://github.com/c2nes/javalang) 做语法解析）。
...
```cmd
pip install dist\java2plantuml-0.1.0-py3-none-any.whl
```
...
安装后可直接使用入口命令（输出完整 PlantUML，含 `@startuml` / `@enduml`）：

```cmd
java2plantuml <路径>
```

`<路径>` 可以是**单个 `.java` 文件**，或**包含多个 `.java` 的目录**（会递归扫描）。
...
4. **本地验证安装**（可选）：
   ...
   ```cmd
   pip install dist\java2plantuml-0.1.0-py3-none-any.whl
   java2plantuml C:\some\java\folder
   ```

   CLI 需传入**恰好一个**参数（文件或目录路径）；参数不对时会打印用法说明。
